...
class Html {
   ...
   /*public function __construct() {
    ...
    public function __construct()
    {
        ...
        // Build absolute path to order.xml
        $xmlPath = $this->_base_path . DIRECTORY_SEPARATOR . 'order.xml';

        if (! file_exists($xmlPath)) {
            throw new \Exception("order.xml not found at: {$xmlPath}");
        }

        $XMLData = file_get_contents($xmlPath);
        if ($XMLData === false) {
            throw new \Exception("Unable to read order.xml at: {$xmlPath}");
        }

        $this->XML = simplexml_load_string($XMLData);
        if ($this->XML === false) {
            throw new \Exception("Failed to parse order.xml: {$xmlPath}");
        }

        // Initialize builder parts
        $this->_init_sections();
        $this->_init_fonts(new Fonts());
