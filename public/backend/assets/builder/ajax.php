<?php
	...
	class Request {
				...
				if ( $this->_strposa( strtolower($file_name), $arr ) ) {
					if ($this->_validation($_FILES['data'], 'file')) {
						if ( move_uploaded_file( $_FILES['data']['tmp_name'],
							...
							if ($mode === 'addgallery') {
								echo json_encode( array( 'fileName' => '/public/backend/assets/builder/images/gallery/' . $sub_folder . $file_name ) );
		...
		public function Addgallery() {
			$this->_upload_file($this->_base_path .'/public/backend/assets/builder/images/gallery/', array( '.png', '.jpg', '.jpeg', '.gif', '.svg' ), 'addgallery',$_POST['userId'],$_POST['project_id']);
		}
			...
			foreach ($this->_images as $image) {
				...
				if (is_file($base_path. '/' .$image)) {
					if ($type === 'retina') {
                        ...
                        $response['general'][] = array(
							'name'   => $image . '?t=' . time(),
                            'path'   => './public/backend/assets/builder/images/gallery/' . $image
						,	'width'  => $size[0]
						,	'height' => $size[1]
						);
					} else if ($type === 'normal') {
                    ...
                    foreach ($innerImage as $imageL2) {
                        ...
                        if (is_file($base_path. '/' . $image . '/' . $imageL2)) {
                            if ($type === 'retina') {
                                ...
                                $response[$image][] = array(
                                    'name'   => $imageL2 . '?t=' . time(),
                                    'path'   => './public/backend/assets/builder/images/gallery/' . $image . '/' . $imageL2
                                ,	'width'  => $size[0]
                                ,	'height' => $size[1]
                                );
                            } else if ($type === 'normal') {
...
//					}
				...
				if ($page->style !== '') {
					...
					preg_match_all('#(/)?public/backend/assets/builder/images/gallery/([^"\')&]*)#i', $page_style, $customImages);
					if (!empty($customImages[2]) && count($customImages[2]) > 0) {
						foreach($customImages[2] as $image) {
							if (file_exists('public/backend/assets/builder/images/gallery/'. $image)) {
								$zip->addFile( 'public/backend/assets/builder/images/gallery/'. $image, 'images/' . $image );
							}
						}
					}
				...
				preg_match_all('#(/)?public/backend/assets/builder/images/gallery/([^"\'\s]*)#i', $preloader, $customImages);
				if (!empty($customImages[2]) && count($customImages[2]) > 0) {
					foreach($customImages[2] as $image) {
						$zip->addFile( 'public/backend/assets/builder/images/gallery/'. $image, 'images/' . $image );
					}
				...
				preg_match('#(/)?public/backend/assets/builder/images/gallery/(.*)#i', $page->favicon, $favicon);
				$link_favicon = "";
				if (!empty($favicon) && file_exists('public/backend/assets/builder/images/gallery/' . $favicon[2])) {
					$link_favicon = "\n<link rel=\"icon\" href=\"images/$favicon[2]\" type=\"image/x-icon\">";
					$zip->addFile( 'public/backend/assets/builder/images/gallery/' . $favicon[2], 'images/' . $favicon[2] );
				}
    ...
    <body class=\"".$page->style_options."\">$preloader";
				...
				preg_match_all('#(/)?public/backend/assets/builder/images/gallery/([^"\'\)&]*)#i', $content, $customImages);
				if (!empty($customImages[2]) && count($customImages[2]) > 0) {
					foreach($customImages[2] as $image) {
						if (preg_match('/(.*)\s2x/i',$image,$img)) {
							$zip->addFile( 'public/backend/assets/builder/images/gallery/' . $img[1], 'images/' . $img[1] );
						} else {
							$zip->addFile( 'public/backend/assets/builder/images/gallery/' . $image, 'images/' . $image );
						}
					}
...
//								var_dump('video/gallery/' . $video . $type);die;
		...
		private function saveSiteToTmp($dataPost) {
            ...
            preg_match_all('#(/)?public/backend/assets/builder/images/gallery/([^"\')&]*)#i', $data->style, $customImages);
            if (!empty($customImages[2]) && count($customImages[2]) > 0) {
                foreach($customImages[2] as $image) {
                    if (file_exists('public/backend/assets/builder/images/gallery/'. $image)) {
                        $zip->addFile( 'public/backend/assets/builder/images/gallery/'. $image, 'images/' . $image );
